...
import signon.orchestrator.api.VersionApi;

import signon.orchestrator.model.Version;

@RestController
public class SignONOrchestratorVersionController implements VersionApi{

  @Value("${version.orchestrator}")
  private String versionOrchestrator;

  @Value("${version.openapi}")
  private String versionOpenapi;

  @Value("${version.asyncapi}")
  private String versionAsyncapi;

    @Override
    public ResponseEntity<Version> getVersion(){
      Version version = new Version();
      version.orchestrator(versionOrchestrator);
      version.openAPI(versionOpenapi);
      version.asyncAPI(versionAsyncapi);
